...
export type ProfileWorkerResult = { status: "complete"; summary: ProfileImportSummary; fingerprint: string } | { status: "error"; code: ProfileImportError }
...
export function createProfileImportController(input: {
  ...
  const operation = async (run: () => Promise<ProfileImportResult>): Promise<ProfileImportResult> => {
    ...
    busy = true
    try { return await run() } catch { return { status: "error", code: "invalid" } } finally { busy = false }
  }
  return {
    clear(sender: number) { tokens.delete(sender) },
    preview(sender: number, browse: unknown) {
      return operation(async () => {
        tokens.delete(sender)
        const dest = input.destination()
        if (!dest || (browse !== undefined && typeof browse !== "boolean")) return { status: "error", code: "unavailable" }
