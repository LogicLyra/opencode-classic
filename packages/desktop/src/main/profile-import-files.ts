...
export function parseProfileConfig(text: string): Record<string, unknown> {
  ...
  if (errors.length || !value || typeof value !== "object" || Array.isArray(value)) throw new ProfileImportFailure("invalid")
...
export function inventoryProfile(roots: ProfileRoots): ProfileInventory {
  ...
  const walk = (source: string, path: string, git = false, alternates: readonly string[] = []) => {
    ...
    if (basename(source) === ".git") {
      ...
      for (const name of ["HEAD", "index", "config.worktree", "ORIG_HEAD", "MERGE_HEAD", "MERGE_MSG", "COMMIT_EDITMSG"]) {
...
export function copyProfileFiles(inventory: ProfileInventory, source: ProfileRoots, target: ProfileRoots, stage: string) {
  ...
  for (const entry of inventory.entries) {
    ...
    if (entry.kind === "directory") { mkdirSync(path, { recursive: true, mode: 0o700 }); continue }
    ...
    const out = openSync(path, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | constants.O_NOFOLLOW, entry.mode | 0o600)
    try {
  ...
  for (const entry of inventory.entries.filter((item) => item.kind === "directory").reverse()) syncDirectory(join(stage, entry.path))
}
