...
type Journal = { version: 1; id: string; phase: "building" | "ready" | "activated"; original: string; database: string; digest?: string }
const marker = ".profile-import-owner"

function directoryIdentity(path: string) {
  const info = lstatSync(path)
  if (!info.isDirectory() || info.isSymbolicLink() || realpathSync(path) !== path) throw new ProfileImportFailure("invalid")
...
export function beginProfileStage(userData: string, database: string): Journal {
  ...
  const journal: Journal = { version: 1, id: randomUUID(), phase: "building", original: directoryIdentity(paths.live), database: basename(database) }
...
function readJournal(userData: string): Journal {
  ...
  if (value.version !== 1 || typeof value.id !== "string" || !/^[a-f0-9-]{36}$/.test(value.id) ||
    !["building", "ready", "activated"].includes(String(value.phase)) || typeof value.original !== "string" || !/^\d+:\d+$/.test(value.original) ||
    typeof value.database !== "string" || !/^opencode(?:-[\w-]+)?\.db$/.test(value.database) ||
    (value.phase !== "building" && (typeof value.digest !== "string" || !/^[a-f0-9]{64}$/.test(value.digest)))) throw new ProfileImportFailure("invalid")
...
export function cancelProfileStage(userData: string, journal: Journal) {
  const paths = profilePaths(userData)
  if (existsSync(paths.backup) || directoryIdentity(paths.live) !== journal.original) throw new ProfileImportFailure("invalid")
  if (existsSync(paths.stage)) {
    directoryIdentity(paths.stage)
    if (readdirSync(paths.stage).length === 0) rmdirSync(paths.stage)
    else { assertOwnedStage(paths.stage, journal); rmSync(paths.stage, { recursive: true }) }
  }
...
export function activateProfileImport(userData: string) {
  ...
  if (journal.phase === "building") { cancelProfileStage(userData, journal); return }
  if (existsSync(paths.backup) && directoryIdentity(paths.backup) !== journal.original) throw new ProfileImportFailure("invalid")
  if (existsSync(paths.stage)) {
    ...
    syncDirectory(userData)
  } else if (journal.phase !== "activated" && profileTreeDigest(paths.live) !== journal.digest) throw new ProfileImportFailure("changed")
...
export function recoverProfileImport(userData: string) {
  ...
  if (!existsSync(paths.journal)) return
  const journal = readJournal(userData)
  try { activateProfileImport(userData) } catch (error) {
