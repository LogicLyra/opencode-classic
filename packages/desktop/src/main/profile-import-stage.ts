...
import { existsSync, lstatSync, mkdirSync, mkdtempSync, readdirSync, realpathSync, renameSync, rmSync, writeFileSync, openSync, fsyncSync, closeSync, constants, statfsSync } from "node:fs"
import { basename, dirname, join } from "node:path"
import type { ProfileImportSummary } from "@opencode-ai/app/profile-import"
import { copyDatabaseSnapshot, copyProfileFiles, databaseFingerprint, inventoryProfile, parseProfileConfig, profileTreeDigest, remapConfig, secureRead, syncDirectory } from "./profile-import-files"
...
export function runProfileImport(input: ProfileImportInput): { summary: ProfileImportSummary; fingerprint: string } {
  ...
  const databaseBytes = [database, `${database}-wal`].reduce((sum, path) => sum + (existsSync(path) ? lstatSync(path).size : 0), 0)
  ...
  if (space.bavail * space.bsize < inventory.bytes * 1.1 + databaseBytes * 4 + 256 * 1024 * 1024) throw new ProfileImportFailure("space")
  const scratch = mkdtempSync(join(input.userData, ".profile-snapshot-"))
  try {
    ...
    const summary: ProfileImportSummary = {
      config: input.source.config, data: input.source.data, state: input.source.state,
      sessions: count("session"), providers: Object.keys(auth).length, accounts: count("account") + count("control_account"), workspaces: count("workspace"), files: inventory.entries.filter((entry) => entry.kind !== "directory").length, bytes: inventory.bytes + databaseBytes,
      plugins: configs.reduce((n, cfg) => n + (Array.isArray(cfg.plugin) ? cfg.plugin.length : 0), 0) + inventory.entries.filter((entry) => /^config\/opencode\/plugins?\/[^/]+\.[cm]?[jt]s$/.test(entry.path)).length,
      mcp: configs.reduce((n, cfg) => n + (cfg.mcp && typeof cfg.mcp === "object" ? Object.keys(cfg.mcp).length : 0), 0),
      commands: Number(source.prepare("SELECT count(*) AS n FROM project WHERE commands IS NOT NULL").get()?.n),
      permissions: count("permission") + Number(source.prepare("SELECT count(*) AS n FROM session WHERE permission IS NOT NULL").get()?.n),
      pending: Number(source.prepare("SELECT count(*) AS n FROM session_input WHERE promoted_seq IS NULL").get()?.n),
    ...
    try {
      ...
      for (const row of source.prepare("SELECT project_id, directory FROM session UNION SELECT project_id, directory FROM workspace UNION SELECT id,worktree FROM project").iterate()) {
    ...
    } finally { closeProfileDatabase(destination) }
    } finally { closeProfileDatabase(source) }
  } catch (error) {
