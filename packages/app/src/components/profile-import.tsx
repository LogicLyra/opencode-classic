...
export function ProfileImportPanel(props: { onBusy?: (busy: boolean) => void } = {}) {
  ...
  createEffect(() => {
    server.key
    setState({ consent: false, result: undefined })
  })
  onMount(() => {
    void platform.profileImport
      ?.status()
      .then((result) => {
        if (!state.result && !state.busy) setState("result", result)
      })
      .catch(() => undefined)
  })
  ...
  return (
    <section class="flex flex-col gap-4 min-w-0" aria-busy={state.busy}>
      ...
      <Show when={local()} fallback={<p role="status">{language.t("chatImport.localOnly")}</p>}>
        <Show when={state.result?.status !== "staged"}>
          <div class="flex flex-wrap gap-2">
            <Button disabled={state.busy} onClick={() => void run(false)}>
              {language.t("profileImport.detect")}
            </Button>
            <Button disabled={state.busy} onClick={() => void run(false, true)}>
              {language.t("profileImport.browse")}
            </Button>
          </div>
        </Show>
          ...
          {(result) => (
            <>
              <dl class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 min-w-0">
                <dt>{language.t("profileImport.data")}</dt>
                <dd class="break-all">{result().summary.data}</dd>
                <dt>{language.t("profileImport.config")}</dt>
                <dd class="break-all">{result().summary.config}</dd>
                <dt>{language.t("profileImport.state")}</dt>
                <dd class="break-all">{result().summary.state}</dd>
                <dt>{language.t("chatImport.total")}</dt>
                <dd>{result().summary.sessions}</dd>
                <dt>{language.t("profileImport.providers")}</dt>
                <dd>{result().summary.providers}</dd>
                <dt>{language.t("profileImport.accounts")}</dt>
                <dd>{result().summary.accounts}</dd>
                <dt>{language.t("profileImport.workspaces")}</dt>
                <dd>{result().summary.workspaces}</dd>
                <dt>{language.t("profileImport.files")}</dt>
                <dd>{result().summary.files}</dd>
                <dt>{language.t("profileImport.bytes")}</dt>
                <dd>{result().summary.bytes}</dd>
                <dt>{language.t("profileImport.plugins")}</dt>
                <dd>{result().summary.plugins}</dd>
                <dt>{language.t("profileImport.mcp")}</dt>
                <dd>{result().summary.mcp}</dd>
                <dt>{language.t("profileImport.commands")}</dt>
                <dd>{result().summary.commands}</dd>
                <dt>{language.t("profileImport.permissions")}</dt>
                <dd>{result().summary.permissions}</dd>
                <dt>{language.t("profileImport.pending")}</dt>
        ...
        <Show when={state.result?.status === "staged"}>
          <Button variant="primary" onClick={() => void platform.restart()}>
            {language.t("profileImport.restart")}
          </Button>
        </Show>
      </Show>
    </section>
