...
export const chatImportEnglish = {
  ...
  "profileImport.description": "Copy your compatible OpenCode setup into an empty Classic desktop profile: chats, provider credentials, cloud accounts, permissions, global configuration, agents, skills, plugins, plans, snapshots and workspace files. The source stays unchanged. Preview does not execute imported commands or contact providers.",
  "profileImport.boundaries": "Close OpenCode and stop editing its files first. Project folders outside OpenCode storage stay at their original paths. Environment variables, system-installed tools and upstream desktop window preferences are not copied. Logs, caches and process locks are regenerated. OAuth providers may require sign-in again. Custom folders are selected in order: data, config, then state.",
  ...
  "profileImport.staged": "The setup is staged and verified. Restart Classic to activate it before its server starts. Do not add data to Classic before restarting; activation checks again that the destination is empty.",
  "profileImport.activated": "The full setup was activated successfully. Your project folders remain available at their original paths; imported internal workspaces have independent copies.",
  ...
  "profileImport.error.nonempty": "Classic already contains setup data. Full import will not overwrite it. Use Chats only to merge compatible conversations, or start with an empty Classic profile.",
  "profileImport.error.incompatible": "The source database schema does not match this Classic version. Full import requires a compatible SQLite setup; no source migration was attempted.",
